...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        net_u, training_loss_record, u_pre_record = train_lamb_oseen(cfg, net_u)
        torch.save(net_u, './model/lamb_oseen_net_' + str(i))
        torch.save(training_loss_record, './training_loss/lamb_oseen_training_loss_' + str(i))
        torch.save(u_pre_record, './test_loss/lamb_oseen_test_loss_' + str(i))
        u_pre = net_u[-1](xy_grid)
        u_real = sol[-1, :, :]
